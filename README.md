...
├── README.md
├── requirements.txt
├── logistic_regression.ipynb                   # End-to-end walkthrough on synthetic data
...
The notebook uses **synthetic** binary classification data generated by `sklearn.datasets.make_classification` — 2000 samples, 12 features (6 informative, 2 redundant), with `weights=[0.85, 0.15]` so the positive class is the minority. The deliberate class imbalance is what makes the precision / recall / ROC-AUC / threshold-tuning discussion below relevant — accuracy alone would say "predict the majority class for everything" and score ~85%.

To swap in your own data, replace cell 2 of the notebook with `pd.read_csv(...)` or similar — the rest of the pipeline (split, scale, RFE, automated selection, all three model builders, full diagnostic suite) is dataset-agnostic.
