...
├── logistic_regression/                         # the package (importable)
│   ├── __init__.py
...
```bash
git clone https://github.com/modelverseml/Logistic-Regression.git
cd Logistic-Regression
pip install -r requirements.txt

# Run the full walkthrough (data -> feature selection -> 3 models -> metrics)
python main.py

# Add the diagnostic plots (confusion matrix, ROC, PR, threshold sweep)
python main.py --plot
```

## Dataset

The walkthrough uses **synthetic** binary classification data from `sklearn.datasets.make_classification`: 2000 samples, 12 features (6 informative, 2 redundant), with `weights=[0.85, 0.15]` so the positive class is the minority. The deliberate class imbalance is what makes the precision / recall / ROC-AUC / threshold-tuning discussion below relevant — accuracy alone would say "predict the majority class for everything" and score ~85%.

To swap in your own data, replace `build_dataset()` in [`examples/walkthrough.py`](examples/walkthrough.py) with `pd.read_csv(...)` or similar. The rest of the pipeline (split, scale, RFE, automated selection, all three model builders, full diagnostic suite) is dataset-agnostic.
...
The three fitting wrappers (from-scratch GD, scikit-learn, statsmodels) share the same `build_model / predict / predict_proba / get_parameters` interface, so they can be swapped freely. Import them from the `logistic_regression` package:
...
```python
from logistic_regression import ClassificationMetrics
...
```python
from logistic_regression import RfeClass, final_data, VIF
...
This is exactly what [`logistic_regression_model_building.py`](logistic_regression/logistic_regression_model_building.py) implements — that's the entire training loop.
